// V5.2 — Terminal / Amber CRT
// Warm phosphor monochrome palette. Tighter retro aesthetic — slight
// scanline overlay, amber-on-near-black, more all-caps. Same structure as
// V5.1 so it reads as a direct sibling.
...
const V5Amber = (() => {
  ...
  const t = {
    ...
    fg: '#ffb86c',
    mid: '#d18345',
    dim: 'rgba(255,184,108,0.55)',
    faint: 'rgba(255,184,108,0.26)',
    grid: 'rgba(255,184,108,0.14)',
    glow: 'rgba(255,184,108,0.18)',
  ...
  function Scanlines() {
    // Two-layer effect: very faint horizontal scanlines + a soft vignette
    // for a CRT feel without being kitsch.
    return (
      <>
        <div style={{ position: 'absolute', inset: 0, background: 'repeating-linear-gradient(to bottom, rgba(0,0,0,0) 0, rgba(0,0,0,0) 2px, rgba(0,0,0,0.18) 2px, rgba(0,0,0,0.18) 3px)', pointerEvents: 'none', mixBlendMode: 'multiply', zIndex: 15 }} />
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(120% 90% at 50% 50%, transparent 60%, rgba(0,0,0,0.55) 100%)', pointerEvents: 'none', zIndex: 14 }} />
      </>
    );
  }
  ...
  function StatusBar({ now }) {
    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', padding: '10px 16px', borderBottom: `1px solid ${t.grid}`, background: t.panel, fontSize: 11, letterSpacing: 1.4, textShadow: `0 0 6px ${t.glow}` }}>
        <div style={{ color: t.mid }}>[ WALLCAL :: PT1 ]</div>
        <div style={{ color: t.fg, fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>{date}  {h}:{m}:{s}</div>
        <div style={{ color: t.mid, textAlign: 'right' }}>◉ SYNC</div>
      </div>
    );
  }
  ...
  function Header({ now }) {
    ...
    return (
      <div style={{ padding: '14px 18px 12px', borderBottom: `1px solid ${t.grid}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12 }}>
          <div style={{ fontSize: 46, fontWeight: 700, color: t.fg, letterSpacing: -1.4, lineHeight: 0.9, fontVariantNumeric: 'tabular-nums', textShadow: `0 0 10px ${t.glow}` }}>
            {day}.{dnum}
          </div>
          <div style={{ textAlign: 'right', fontSize: 10, color: t.dim, lineHeight: 1.55, letterSpacing: 0.5 }}>
            <div>MONTH={month}</div>
            <div>DOY={dow}/365</div>
            <div>WK19/2026</div>
          </div>
  ...
  function DayGraph({ now }) {
    ...
    for (let h = 0; h < 24; h++) {
      ...
      const ev = events.find((e) => { const sm = U.parseHM(e.start), em = U.parseHM(e.end); return sm < hEnd && em > hStart; });
      const isNow = nowMin >= hStart && nowMin < hEnd;
      cells.push({ h, ev, isNow });
    }
    return (
      <div style={{ padding: '12px 18px', borderBottom: `1px solid ${t.grid}` }}>
        <div style={{ fontSize: 9.5, color: t.dim, letterSpacing: 1.5, marginBottom: 8 }}>▸ DAY/24h</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(24, 1fr)', gap: 2 }}>
          {cells.map((c) => (
            <div key={c.h} style={{
              height: 16,
              background: c.ev ? c.ev.color : 'rgba(255,184,108,0.08)',
              outline: c.isNow ? `1px solid ${t.fg}` : 'none',
      ...
      <div style={{ position: 'relative', padding: '10px 18px 10px 50px', borderBottom: `1px solid ${t.grid}`, flex: 1, overflow: 'hidden' }}>
        ...
        {events.map((e) => {
          ...
          return (
            <div key={e.id} onClick={() => onTap(e)}
              ...
              <span style={{ fontSize: 11, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', textShadow: `0 0 6px ${t.glow}` }}>{e.title}</span>
        ...
        {nowMin >= DAY_START && nowMin <= DAY_END && (
          <div style={{ position: 'absolute', left: 18, right: 18, top: mToY(nowMin) + 14, pointerEvents: 'none' }}>
            <div style={{ position: 'absolute', left: 32, right: 0, height: 1, background: t.fg, boxShadow: `0 0 6px ${t.fg}` }} />
            <div style={{ position: 'absolute', left: 28, top: -3, width: 6, height: 6, background: t.fg, boxShadow: `0 0 6px ${t.fg}` }} />
            <div style={{ position: 'absolute', right: 0, top: -12, fontSize: 9, color: t.fg, letterSpacing: 0.6, fontWeight: 700 }}>
    ...
    return (
      <div style={{ padding: '10px 14px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
          <span style={{ fontSize: 24, fontWeight: 700, color: t.fg, fontVariantNumeric: 'tabular-nums', letterSpacing: -0.6, textShadow: `0 0 8px ${t.glow}` }}>{weather.now.temp}°</span>
        ...
        <div style={{ marginTop: 8 }}>
          <svg viewBox="0 0 100 100" preserveAspectRatio="none" width="100%" height="38">
            <path d={path} stroke={t.fg} strokeWidth="1.2" fill="none" vectorEffect="non-scaling-stroke" />
      ...
      <div style={{ padding: '10px 14px', borderLeft: `1px solid ${t.grid}` }}>
        ...
        {next ? (
          <>
            ...
            <div style={{ marginTop: 8, fontSize: 18, color: t.fg, fontWeight: 700, fontVariantNumeric: 'tabular-nums', letterSpacing: -0.4, textShadow: `0 0 8px ${t.glow}` }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          {tasks.slice(0, 5).map((task) => (
            <div key={task.id} style={{ fontSize: 11, color: task.done ? t.faint : t.fg, display: 'flex', gap: 6 }}>
              <span style={{ color: task.done ? t.faint : t.mid }}>[{task.done ? '×' : ' '}]</span>
  ...
  function EventDetail({ event, onClose }) {
    ...
    return (
      <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(13,8,5,0.88)', zIndex: 30, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, fontFamily: t.mono }}>
        <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', border: `1px solid ${event.color}`, background: t.panel, padding: 18 }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 700, color: event.color, marginTop: 10, letterSpacing: -0.3, textShadow: `0 0 8px ${event.color}66` }}>{event.title}</div>
